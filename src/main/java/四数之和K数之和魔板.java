...
public class 四数之和K数之和魔板 {
     ...
     * @return 系统要求的返回值
     */
    public static List<List<Integer>> sun(int[] nums, int target, int k) {
        // 先排序，这个是必须的。
        Arrays.sort(nums);
        ...
        int begin = 0;
        // 递归开始
        List<List<Integer>> ans = ksum(nums, stack, stack_index, k, begin, target);
        // 递归结束，返回解
        return ans;
    }
     ...
     * @param stackIndex 栈指针，初始化值为-1
     * @param k 表明当前问题被 分解/递归 成了 k数之和 的问题
     * @param begin 当前问题要固定的值的起点
     * @param target 当前 k数之和 的目标和
     * @return 当前 k数之和 的解集，要在上一层合并到最终解集里去
     */
    private static List<List<Integer>> ksum(int[] nums, int[] stack, int stackIndex, int k, int begin, int target){
        ...
        if(k == 2){
            ...
            for(int i = begin; i < nums.length - k + 1; i++){
                ...
                // 递归调用 k - 1 数之和 问题的求解
                List<List<Integer>> ans_temp = ksum(nums,stack, stackIndex, k - 1, i + 1, targetTemp);
    ...
    public static void main(String[] args) {
        ...
        int k = 4;
        int target = 0;
        System.out.println(JSONObject.toJSONString(四数之和K数之和魔板.sun(input,target,k)));
    }
}
